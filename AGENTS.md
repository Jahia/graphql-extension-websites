...
| `PathSecurity` | Package-private path-containment helper (`resolveContained`, `isContained`, `toRealPathOfExistingAncestor`); resolves untrusted GraphQL path fragments against a trusted base and rejects any that escape it. Used by `exportWebsite` and `importWebsite` |
...
| `exportWebsite` | `(siteKey, exportPath, onlyStaging)` → Boolean | `@GraphQLAsync`; `exportPath` is relative to `{jahiaVarDiskPath}/exports/`, must stay inside it (`PathSecurity.resolveContained`) **and must not be it** (SEC-363) |
...
- **Containment is `startsWith`, so the base contains itself — a destructive caller must check base-equality separately (SEC-363).** `exportWebsite` clears whatever sits at `exportPath` before exporting (Jahia refuses a non-empty server export directory), and that clear is a recursive `FileUtils.deleteQuietly`. `exportPath: "."` resolves to `{jahiaVarDiskPath}/exports` itself, passes `PathSecurity.resolveContained` — it never escaped — and, up to and including 2.2.0, deleted **every site's** export artifacts while returning `true`. `PathSecurity` is correct and must not be touched: it answers *"did you escape?"*, and `importWebsite` legitimately resolves a path that may equal its base. The fix is two independent `equals` refusals in `WebsitesAdminMutation` — one in `exportWebsite` (before the site is even resolved, so it leaks nothing) and one inside `deleteExportArtifact`, which now takes a `protectedBase` it may never delete. Do **not** collapse them into one; a single call-site check is one refactor away from being gone. Both are pinned by `WebsitesAdminMutationExportDeletionGuardTest` and `WebsitesAdminMutationDeleteExportArtifactTest`. `PathSecurityTest.resolveContained_dotOnlyChild_resolvesToBase_whichCallersMustGuardAgainstThemselves` still asserts `"."` is *contained* — that is true of the helper and was never the bug; read its javadoc before "fixing" it
